 ...
 public:
  ...
  void RobotInit() {
    ...
    m_motor.SetInverted(true);
		...
		m_motor.Config_kP(kPIDLoopIdx, 2.3, kTimeoutMs);
		m_motor.Config_kI(kPIDLoopIdx, 0.0, kTimeoutMs);
		m_motor.Config_kD(kPIDLoopIdx, 4.0, kTimeoutMs);
	}
  ...
  void TeleopPeriodic() override { 
		...
		if (!_lastButton1 && button1) {
			...
			targetPositionRotations =  10 * 4096; /* 10 Rotations in either direction */
